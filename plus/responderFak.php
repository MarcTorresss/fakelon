        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if (isset($_POST['textfak']) && !empty($_POST['textfak'])) {
                ...
                if ($textfak != null) {
                    ...
                    $db = openDB();
                    $sql = 'INSERT INTO `fak`(Text,Data_Publicacio,ImatgeURL,NomUser,IdOG) 
                    VALUES(:text,now(),:img,:user,:IdOG)';
                    $fak = $db->prepare($sql);
                    if($uploadOk==1){
                        $fak->execute(array(':text' => $textfak, ':img' => 'http://localhost/imagenes/'.$hash.'.'.$imageFileType, ':user' => $username, ':IdOG' => $_POST['IdOG']));
                    }else {
                        $fak->execute(array(':text' => $textfak, ':img' => null, ':user' => $username, ':IdOG' => $_POST['IdOG']));                        
                    }
